...
class NedborUtil:
    ...
    def getNedborFiles(self):
        out = []    
        dirlist = listdir(self.path)
        for item in dirlist:
            if isfile(join(self.path,item)) and item.endswith('.csv') and item.startswith('n'):            
                out.append(item)  
        out.sort()                      
        return out   
    
    def nedborToDataframe(self, **params):
        csvfiles = self.getNedborFiles()    
        index = []
        column = []
    ...
    def saveDataFrame(self,dataframe,lang="None"):
        if lang=="da":
            file = self.path + 'alldata_da.csv'
        else:
            file = self.path + 'alldata.csv'    
        data = dataframe.to_csv(sep=';',index_label='periode')        
        fhandle = open(file,'w',newline='')
        fhandle.write(data)
        fhandle.close() 
    
    def saveDataFrameHtml(self,dataframe):
        file = self.path + 'alldata.html'
        data = dataframe.to_html(border=0)            
        fhandle = open(file,'w',newline='')
        fhandle.write(data)
        fhandle.close()       
        
    def saveAllNedborData(self, **params):
        language = params["lang"] if "lang" in params else "None"
        df = self.nedborToDataframe(lang=language)
        type = params["type"] if "type" in params else "csv"
        if type=="html":
            self.saveDataFrameHtml(df)
        else:                         
            self.saveDataFrame(df, language)      
            
    def makeHtml(self,htmlpath, dataframe, flip=False):
        if not flip:
            ...
            tablehtml = dataframe.style.format(precision=1, decimal=",")\
                ...
                .map(lambda v: 'color:lightgray;background-color:lightgray;' if math.isnan(v) else None,subset=monthcolumns)\
